...
음성 인식(ASR) 시스템을 개발하다 보면 언젠가는 반드시 마주치게 되는 벽이 있다. 바로 **"이 복잡한 WFST 기반 디코딩 그래프는 도대체 어떻게 작동하는 걸까?"** 라는 의문이다. 특히 Kaldi의 HCLG 그래프나 WeNet의 TLG 디코딩 방식은 논문만 읽어서는 완전히 이해하기 어렵고, 코드를 직접 분석해야만 그 비밀이 드러나는 경우가 많다.

이 글에서는 단순히 "WFST가 무엇인가"에 대한 기본 설명을 넘어, Kaldi와 WeNet에서 구현된 WFST 기반 디코딩 방식의 내부 구조와 작동 원리를 실제 예시와 함께 파헤쳐보려 한다. <!--more-->
...
- **전이(Transition)**: 하나의 상태에서 다른 상태로 이동하는 간선
...
음성 인식에서 WFST는 음향 모델(Acoustic Model)의 출력을 단어 시퀀스로 변환하는 복잡한 과정을 효율적으로 처리하는 데 사용된다. 이 과정에서 다음과 같은 변환 단계가 필요하다:
...
WFST 프레임워크의 강력함은 다양한 연산을 통해 트랜스듀서를 조작하고 최적화할 수 있다는 점에 있다. 주요 연산으로는:

- **합성(Composition) ∘**: 두 트랜스듀서를 연결하여 하나의 새로운 트랜스듀서 생성
...
Kaldi는 현재까지도 많은 ASR 시스템의 기반이 되는 오픈 소스 툴킷으로, WFST 기반 디코딩의 표준을 확립했다고 볼 수 있다. Kaldi의 디코딩 그래프는 `HCLG.fst`라는 이름으로 불리는데, 이는 그래프를 구성하는 네 가지 주요 트랜스듀서의 합성을 의미한다:
...
`H` 트랜스듀서는 **HMM 상태(또는 PDF-ID)를 문맥 의존적 음소(CD-phone)로 변환**하는 역할을 한다. 이는 음향 모델의 출력과 발음 모델 사이의 연결고리이다.
...
이 예시에서:
- 상태 0에서 상태 1로의 전이는 HMM 상태 1을 입력으로 받아 문맥 의존적 음소 11001을 출력하며 가중치는 0.4
...
`C` 트랜스듀서는 **문맥 의존적 음소(CD-phone)를 기본 음소(basephone)로 변환**하는 역할을 한다. 이는 음향 모델에서 사용하는 풍부한 문맥 정보를 발음 사전에서 사용하는 기본 음소로 매핑한다.
...
이 예시에서:
- 문맥 의존적 음소 11001은 기본 음소 p_a로 매핑됨 (p 다음에 오는 a를 의미)
- 문맥 의존적 음소 11002는 기본 음소 a로 매핑됨
...
`L` 트랜스듀서는 **음소 시퀀스를 단어로 변환**하는 발음 사전(lexicon)을 나타낸다. 이는 단어의 다양한 발음 변이를 처리한다.
...
이 예시에서:
- 음소 시퀀스 "p_a a n"은 단어 "PANDA"로 매핑됨
- <eps>는 입실론(epsilon) 전이로, 입력 또는 출력 없이 상태 전이가 가능함을 의미

L 트랜스듀서는 동일한 단어에 대한 여러 발음 변이를 포함할 수 있다:
...
여기서 가중치(예: -0.69, -1.61)는 각 발음 변이의 확률을 나타낸다. 값이 작을수록 더 높은 확률을 의미한다.
...
`G` 트랜스듀서는 **단어 시퀀스의 확률을 모델링**하는 언어 모델을 나타낸다. 일반적으로 n-gram 언어 모델을 WFST 형태로 변환한 것이다.
...
- 가중치는 로그 확률을 나타내므로, 값이 작을수록 더 높은 확률을 의미

실제 G 트랜스듀서는 훨씬 더 복잡하며, 백오프(backoff) 아크를 포함하여 학습 데이터에서 보지 못한 n-gram도 처리할 수 있다.

```python
# G 트랜스듀서 생성 의사 코드 (bigram 예시)
...
이제 네 개의 트랜스듀서가 어떻게 하나의 `HCLG.fst`로 합성되는지 살펴보자.
...
단순히 네 개의 트랜스듀서를 합성하는 것으로는 효율적인 디코딩 그래프를 얻을 수 없다. Kaldi는 다양한 최적화 기법을 적용한다:
...
전통적인 HMM-GMM/DNN 기반 시스템과 E2E 시스템의 가장 큰 차이점은 출력 단위에 있다:
...
`T` 트랜스듀서는 **E2E 모델의 출력 토큰을 발음 사전의 입력 단위로 변환**하는 역할을 한다. Kaldi의 H, C와 유사하지만 훨씬 단순하다.
...
WeNet의 `L`과 `G` 트랜스듀서는 Kaldi의 것과 개념적으로 동일하다. 다만 E2E 시스템의 특성에 맞게 일부 조정이 있을 수 있다.

- **L 트랜스듀서**: 음소 시퀀스를 단어로 변환
- **G 트랜스듀서**: 단어 시퀀스의 확률 모델링
...
#### 2. 외부 언어 모델과 E2E 모델의 결합

WeNet의 TLG 디코딩은 E2E 모델의 출력과 외부 언어 모델을 효과적으로 결합한다. 이를 위해 일반적으로 다음과 같은 점수 결합 방식을 사용한다:

```
Score = α * log(P_am) + β * log(P_lm) + γ * word_insertion_penalty
```

여기서:
- `P_am`: E2E 모델(음향 모델)의 토큰 확률
- `P_lm`: 언어 모델의 단어 확률
- `α`, `β`, `γ`: 가중치 하이퍼파라미터
...
```cpp
// WeNet TLG 디코딩 의사 코드 (간략화됨)
for each token_step in e2e_output:
    token_probabilities = e2e_model_output[token_step]
    
    for each active_token in active_tokens:
        state = active_token.state
        for each arc in fst.arcs(state):
            input_symbol = arc.input_symbol
            if input_symbol in token_probabilities:
                acoustic_score = log(token_probabilities[input_symbol])
                combined_score = alpha * acoustic_score + arc.weight  // arc.weight는 이미 LM 점수 포함
                
                next_state = arc.next_state
                new_token = Token(next_state, combined_score, output_symbol=arc.output_symbol)
                add_to_next_tokens(new_token)
...
E2E 시스템에서는 디코딩 스텝이 적기 때문에, 더 넓은 빔을 사용하더라도 전체 디코딩 시간이 크게 증가하지 않는다. WeNet은 이를 활용한 최적화 전략을 채택한다:

- **히스토그램 프루닝(Histogram Pruning)**: 각 상태에서 상위 N개의 토큰만 유지
- **적응적 빔 조정**: 토큰 분포에 따라 빔 너비를 동적으로 조정

```cpp
// 히스토그램 프루닝 의사 코드
void HistogramPruning(vector<Token>& tokens, int max_tokens) {
    if (tokens.size() <= max_tokens) return;
    
    // 비용 기준 정렬
    sort(tokens.begin(), tokens.end(), 
         [](const Token& a, const Token& b) { return a.cost < b.cost; });
...
WeNet의 TLG 디코딩은 속도를 위해 다양한 최적화 기법을 도입했다:

- **동적 그래프 확장(On-the-fly Graph Expansion)**: 필요한 부분만 메모리에 로드
- **탐색 공간 제한**: 토큰 히스토리 기반 그룹화로 중복 탐색 방지
- **병렬 디코딩**: 여러 발화를 병렬로 처리

```cpp
// 동적 그래프 확장 의사 코드
class LazyFstExpander {
public:
    vector<Arc> GetArcs(int state_id) {
        if (expanded_states_.find(state_id) != expanded_states_.end()) {
            return expanded_states_[state_id];
        }
        
        // 필요시 상태 확장
        vector<Arc> arcs = ExpandState(state_id);
        expanded_states_[state_id] = arcs;
        return arcs;
    }
    
private:
    unordered_map<int, vector<Arc>> expanded_states_;
};
```

### TLG vs HCLG: 실전 비교

WeNet의 TLG와 Kaldi의 HCLG를 실제 사용 측면에서 비교해보자:
...
TLG는 E2E 모델의 출력이 이미 고수준 단위(음소 또는 그 이상)이므로 H, C 부분이 크게 단순화되어 전체 그래프 크기가 획기적으로 줄어든다.
...
어떤 방식을 선택하든, WFST 기반 디코딩의 핵심 원리를 이해하는 것은 ASR 시스템 개발자에게 필수적인 지식이다. 이 글이 복잡한 WFST 디코딩 그래프의 내부 구조를 이해하는 데 도움이 되었기를 바란다.
