...
## 방출 확률($b_j(o_t)$), 상태 전이 확률($a_{ij}$)
그렇다면 대체 '방출 확률($b_j(o_t)$)'과 '상태 전이 확률($a_{ij}$)' 이라는 것은 어떻게 구할까?!
...
### 방출 확률($b_j(o_t)$) 구하기
GMM은 여러 가우시안 분포의 가중 합으로 표현되는 확률 밀도 함수(PDF)다. HMM-GMM 시스템에서는 각 HMM 상태의 방출 확률을 GMM으로 모델링한다:<br>
각 HMM 상태 $j$의 방출 확률은 가우시안 혼합 모델로 표현:<br>
$b_j(o_t) = \sum_{m=1}^M c_{jm} \mathcal{N}(o_t; \mu_{jm}, \Sigma_{jm})$<br>
